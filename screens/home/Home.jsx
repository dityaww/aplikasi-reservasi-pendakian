...
import { filterbyCategory, getDataPost } from "../../hook/api";
import LottieView from "lottie-react-native";
import { allData } from "../../redux/Content/allpostSlice";
...
const Home = () => {
  ...
  useEffect(() => {
    const Alldata = async () => {
      try {
        const response = await getDataPost();
  ...
  }, [dataPost, activeFilter]);



  const byCategory = async (q) => {
    const response = await filterbyCategory(q);
    setDataPost(response.data);
    setIsLoading(false);
  };
            ...
            return (
              <Pressable
                ...
                onPress={() => {
                  ...
                  if (items.kategori !== "All") {
                    byCategory(items.kategori.toLowerCase());
                  }
                }}
                style={[
